...
/**
 * Mutator adapter for OnlineMenu API hooks (`customInstance`).
 *
 * Post-BFF-cutover this routes same-origin to `/bff/api/menus/*`; the
 * `bff-erevna` reverse proxy attaches the `Bearer` server-side and forwards
 * to `onlinemenu-api`. `withToken` is gone — the SPA holds no token.
 */

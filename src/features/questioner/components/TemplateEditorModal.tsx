...
import { toRespondentContactMode } from '../utils/respondentContact';
 ...
 */
function createUpdatePayload({ name, description, isActive, contents, itemContents, closingDate, maxResponses, respondentContactMode }: UpdatePayloadParams): UpdateQuestionerTemplateRequest {
...
const TemplateEditorModal = ({ visible, item, onCancel, onSave, enableAnswerSection = false, readOnly = false }: Props): React.ReactElement => {
  ...
  const [respondentContactMode, setRespondentContactMode] = useState<RespondentContactMode>(
    () => toRespondentContactMode(item?.respondentContactMode),
  );
  ...
  useLayoutEffect(() => {
    ...
    if (!isValueDefined(item)) {
      ...
      setClosingDate('');
      setMaxResponses('');
      setRespondentContactMode(RespondentContactMode.Anonymous);
      return;
    }
    ...
    setRespondentContactMode(toRespondentContactMode(item.respondentContactMode));
    setActiveTab('form');
  }, [item, visible]);
  ...
  function handleSaveFromForm(): void {
    const payload = createUpdatePayload({ name, description, isActive, contents, itemContents: item?.contents, closingDate, maxResponses, respondentContactMode });
    onSave(payload);
  }
  ...
  function handleSaveFromJson(): void {
    ...
    const payload = createUpdatePayload({ name, description, isActive, contents: parsed, itemContents: item?.contents, closingDate, maxResponses, respondentContactMode });
    onSave(payload);
  }

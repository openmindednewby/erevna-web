 ...
 * mocking it would only prove erevna calls a mock. Only the client and the
 * navigation seam are faked, so what is under test is erevna's genuine wiring.
 */
...
// The navigation seam: erevna's own login-route redirect.
...
describe('AuthProvider.logout — the IdP logout URL', () => {
  it('navigates to the IdP URL when one is returned, instead of the local login route', async () => {
    ...
    const idpUrl = 'https://identity.example.test/realms/questioner/protocol/openid-connect/logout?id_token_hint=x';

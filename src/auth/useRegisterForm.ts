...
import { type BffRegisterRequest } from '@dloizides/auth-client';
...
interface RegisterFormApi {
  ...
  submit: () => Promise<{ ok: boolean; errorMessage?: string }>;
}
...
export function useRegisterForm(): RegisterFormApi {
  ...
  const submit = useCallback(async (): Promise<{ ok: boolean; errorMessage?: string }> => {
    ...
    try {
      await register(buildRequest(trimmed));
      return { ok: true };
    } catch (error) {

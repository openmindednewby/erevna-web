/**
 * Login route — unified-auth Phase 5 cutover + unified-login Increment 3 Batch 4.
 *
 ...
 * Credentials are POSTed to the same-origin `bff-erevna`, which terminates
 * auth server-side and sets the httpOnly session cookie — no Keycloak redirect.
 *
 * Increment 3 Batch 4 adds two shared-package surfaces, both driven by the
 ...
 * it's hot when the new route mounts; (3) navigate to the protected area (erevna
 * routes by area, not by role — the per-role split happens inside `/(protected)`).
 */
...
import { bffAuthClient } from '../../src/auth/bffAuthClient';
...
const LoginScreen = (): React.ReactElement => {
  ...
  const handleSignedIn = useCallback(
    (user: BffUser): void => {
      ...
      // Erevna routes by area, not by role: the protected layout decides where to land.
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.colors.background }]} testID="erevna-login-page">
      <PreferredMethodHint
      ...
      {routing ? (
        <Text style={[themeStyles.loadingText, styles.routing]} testID="erevna-login-routing">
          {FM('loading')}
        </Text>
      ) : null}
